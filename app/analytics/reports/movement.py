...
def get_frequency_data(filters):
    ...
    ).join(Product).filter(*filters).group_by(Product.id, Product.name).order_by(desc('count')).all()
...
def get_top_issued_volume(filters):
    ...
    ).join(Product).filter(*out_filters).group_by(Product.id, Product.name).order_by(desc('qty')).all()
...
def get_consistent_pressure(filters):
    ...
    ).join(Product).filter(*out_filters).group_by(Product.id, Product.name).order_by(desc('hits')).all()
...
def get_top_stocked_in(filters):
    ...
    results = db.session.query(
        Product.name,
        func.sum(Transaction.quantity).label('qty')
    ).join(Product).filter(*in_filters).group_by(Product.id, Product.name).order_by(desc('qty')).all()
